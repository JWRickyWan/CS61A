...
def balanced(m):
    ...
    if is_planet(end(left(m))) and is_planet(end(right(m))):
        ...
        if size(end(left(m)))*length(left(m)) == total_weight(end(right(m)))*length(right(m)) and balanced(end(right(m))):
            return True
        else:
            return False
    elif is_mobile(end(left(m))) and is_planet(end(right(m))):
        if size(end(right(m)))*length(right(m)) == total_weight(end(left(m)))*length(left(m)) and balanced(end(left(m))):
            return True
        else:
            return False
    else:
        if total_weight(end(left(m)))*length(left(m)) == total_weight(end(right(m)))*length(right(m)) and balanced(end(right(m)))and balanced(end(left(m))):
            return True
        else:
            return False
...
def quadratic(x, a, b, c):
    ...
    if -b/(2*a) > lower_bound(x) and -b/(2*a)< upper_bound(x):
        if a > 0:
            return interval(ft(-b/(2*a)),max(ft(lower_bound(x)),ft(upper_bound(x))))        
        else:
            return interval(min(ft(lower_bound(x)),ft(upper_bound(x))),ft(-b/(2*a)))
    else:
