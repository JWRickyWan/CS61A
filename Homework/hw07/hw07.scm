...
(define (derive-sum expr var)
  (cond ((number? expr) 0 )
  (else (make-sum (derive (first-operand expr) var) (derive (second-operand expr) var))))
)

(define (derive-product expr var)
  (cond ((number? expr) 0)
  (else (make-sum (make-product (derive (first-operand expr) var) (second-operand expr)) 
...
(define (exp? exp)
  (cond ((number? exp) '#f)
        ((not(list? exp)) '#f)
        ((and (not (number? (first-operand exp))) (number? (second-operand exp))) '#t))
)
...
(define (derive-exp exp var)
  (cond ((not (exp? exp)) 0)
  (else (make-product (second-operand exp)
