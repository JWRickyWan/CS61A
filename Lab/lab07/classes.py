...
class TACard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        300
        """
        result = other_card.attack
        other_card.attack = other_card.defense
        other_card.defense = result
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for i in range(orig_opponent_deck_length):
            if lst[i].attack != other_card.attack and lst[i].defense != other_card.defense:
                nlst.append(lst[i])
        opponent.deck.cards = nlst[:]
